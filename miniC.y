...
method_definition: type TK_ID '(' parameters_type_list ')' block_statement { //void hola(int x){}
                    $$ = new MethodDefinition((Type)$1, $2, *$4, $6, yylineno );
                    delete $4;
                 }
                 | type TK_ID '(' ')' block_statement{ // void hola(){}
                     ParameterList * pm = new ParameterList;
                     $$ = new MethodDefinition((Type)$1, $2, *pm, $5, yylineno );
                     delete pm;
                 }
                 | type TK_ID '(' parameters_type_list ')' ';'{ //void hoal(bool y);
                     $$ = new MethodDefinition((Type)$1, $2, *$4, NULL, yylineno);
                 }
                 | type TK_ID '(' ')' block_statement ';'{ // void gola();
                     ParameterList * pm = new ParameterList;
...
parameter_declaration: type declarator { $$ = new Parameter((Type)$1, $2, false, yylineno); } //(int hola, bool x)
                     | type { $$ = new Parameter((Type)$1, NULL, false, yylineno); } // (int, int, bool)
                     | type '[' ']'  { $$ = new Parameter((Type)$1, NULL, true, yylineno); } //( int [])
                    ;
...
if_statement: TK_IF '(' expression ')' statement {$$ = new IfStatement($3, $5, yylineno);} // if (expression) { block}
            | TK_IF '(' expression ')' statement TK_ELSE statement {$$ = new ElseStatement($3, $5, $7, yylineno);}  //if (expression){ block } else {block}
            ;
